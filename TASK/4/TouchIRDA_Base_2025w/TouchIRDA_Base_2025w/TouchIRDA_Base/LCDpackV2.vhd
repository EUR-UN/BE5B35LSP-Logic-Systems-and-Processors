...
-- useful conditional assignments -------------------------------------------
  ...
  -- Returns TRUE if the pixel (dx, dy) is inside a lit segment of number digit_n
  function CheckSegment(digit_n: integer; dx: integer; dy: integer) return boolean;
...
package body LCDpackV2 is
 ...
 --- conditional assignments 
  ...
  -- Returns TRUE if the pixel (dx, dy) is inside a lit segment of number digit_n
  function CheckSegment(digit_n: integer; dx: integer; dy: integer) return boolean is
      variable is_on : std_logic_vector(6 downto 0); -- 7 segments 
  begin
      -- define which segments are on for each digit use 7-segment display method
      case digit_n is
          when 0 => is_on := "0111111"; 
          when 1 => is_on := "0000110";
          when 2 => is_on := "1011011"; 
          when 3 => is_on := "1001111";
          when 4 => is_on := "1100110"; 
          when 5 => is_on := "1101101";
          when 6 => is_on := "1111101"; 
          when 7 => is_on := "0000111";
          when 8 => is_on := "1111111"; 
          when 9 => is_on := "1101111";
          when others => is_on := "0000000";
      end case;

      -- check each segment
      -- top
      if    is_on(0)='1' and (dy>=0 and dy<4)   and (dx>=4 and dx<16) then return true; 
      -- right top
      elsif is_on(1)='1' and (dy>=4 and dy<18)  and (dx>=16 and dx<20) then return true; 
      -- right bottom
      elsif is_on(2)='1' and (dy>=22 and dy<36) and (dx>=16 and dx<20) then return true; 
      -- bottom
      elsif is_on(3)='1' and (dy>=36 and dy<40) and (dx>=4 and dx<16) then return true; 
      -- left bottom
      elsif is_on(4)='1' and (dy>=22 and dy<36) and (dx>=0 and dx<4) then return true; 
      -- left top
      elsif is_on(5)='1' and (dy>=4 and dy<18)  and (dx>=0 and dx<4) then return true; 
      -- middle
      elsif is_on(6)='1' and (dy>=18 and dy<22) and (dx>=4 and dx<16) then return true; 
